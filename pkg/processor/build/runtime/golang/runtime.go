...
func (g *golang) GetProcessorDockerfileInfo(versionInfo *version.Info) (*runtime.ProcessorDockerfileInfo, error) {
	...
	if g.FunctionConfig.Spec.Build.BaseImage != "" &&
		...
		processorDockerfileInfo.OnbuildImage = "koinotice/handler-builder-golang-onbuild:%s-%s"
	} else {

		// use alpine based image
		processorDockerfileInfo.OnbuildImage = "koinotice/handler-builder-golang-onbuild:%s-%s-alpine"
	}
